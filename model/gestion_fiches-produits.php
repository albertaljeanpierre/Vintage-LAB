<?php
include_once("lib/fiches-produits.php");

$url_page = "gestion_fiches-produits";
...
switch ($get_action) {
    case "liste":
        $page_view = "fiches_produits_liste";
        $alphabet = range('A', 'Z');

        $result = getProduits(0, $get_alpha);

        if (!is_null($get_id) && is_numeric($get_id)) {
            $result_detail = getProduits($get_id);

            $detail_titre = $result_detail[0]["titre"];
...
//            $show_description = true;
    ...
    case "add":
        ...
        $post_etat = isset($_POST["etat"]) ? filter_input(INPUT_POST, 'etat', FILTER_SANITIZE_SPECIAL_CHARS) : null;
        ...
        // Création des tableaux d'option des sélect
        $catSousCat = getCategoriesSousCat();
        $catSousCat_option = array_column($catSousCat, 'catSousCat', 'category_level_2_id');
//        var_dump($catSousCat); exit;

        $shape = getShape();
        $shape_option = array_column($shape, '	shape_title', 'shape_id');
        // var_dump($shape_option);exit;

        $input = [];
        // ajout des différents champs du formulaire
        $input[] = addLayout("<h4>Ajouter un article</h4>");
        $input[] = addLayout("<div class='row'>");
        $input[] = addSelect('Catégorie associée', ['name' => 'categorie', "class" => "u-full-width"], $catSousCat_option, $post_categorie, true, "twelve columns");
        $input[] = addSelect('État de l\'objet', ['name' => 'etat', "class" => "u-full-width"], $shape_option, $post_categorie, true, "twelve columns");

        $input[] = addInput('Nom', ["type" => "text", "value" => $post_nom, "name" => "nom", "class" => "u-full-width"], true, "twelve columns");
        $input[] = addInput('Prénom', ["type" => "text", "value" => $post_prenom, "name" => "prenom", "class" => "u-full-width"], true, "twelve columns");
        $input[] = addLayout("</div>");
        $input[] = addLayout("<div class='row'>");
