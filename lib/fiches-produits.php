...
function getCategoriesSousCat()
{
    $sql = "SELECT category_level_2_id,  concat( LEVEL_1 , \" > \" , LEVEL_2) AS catSousCat , category_level_2.is_visible
                FROM category_level_1 , category_level_2
                WHERE category_level_2.is_visible = '1'
                GROUP BY LEVEL_1 ASC, level_2 ASC ";
    return requeteResultat($sql);
}

function getShape() {
    $sql = "SELECT `shape_id`,`shape_title`,`is_visible`
                FROM `shape`
                WHERE `is_visible` = '1' ";
    return requeteResultat($sql);
}